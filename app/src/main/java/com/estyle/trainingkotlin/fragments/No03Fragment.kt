...
class No03Fragment:BaseFragment() {
    override fun onAnimEnd(animation: Animator) {
        when ((animation as ObjectAnimator).target) {
            rightTwo01TV -> { rightTwo02TV.visibility = View.VISIBLE }
            rightTwo02TV -> { rightTwo03TV.visibility = View.VISIBLE }
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        rightOne01TV.setOnClickListener {
            val two1Anim = AnimatorUtils.createTranslationLeft2Right(rightTwo01TV)
            val two2Anim = AnimatorUtils.createTranslationRight2Left(rightTwo02TV, activity)
            val two3Anim = AnimatorUtils.createTranslationLeft2Right(rightTwo03TV)
